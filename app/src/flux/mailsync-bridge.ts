...
export default class MailsyncBridge {
  ...
  toggleVerboseLogging() {
    const { configDirPath } = AppEnv.getLoadSettings();
    let message = localized('Thank you for helping debug Courier. Courier will now restart.');
    let phrase = 'disabled';
    ...
    if (AppEnv.config.get(VERBOSE_UNTIL_KEY)) {
      ...
      message =
        ...
        `all network traffic to your mail providers and will be quite slow. Restart Courier ` +
        `and wait for your problem to occur, and then submit mailsync-***.log files located ` +
        `in the directory: \n\n${configDirPath}.\n\nCourier will now restart.`;
    }
  ...
  sendMessageToAccount(accountId, json) {
    if (!this._clients[accountId]) {
      ...
      return AppEnv.showErrorDialog({
        title: localized(`Courier is unable to sync %@`, emailAddress),
        message: localized(
  ...
  async resetCacheForAccount(account, { silent }: { silent?: boolean } = {}) {
    ...
    if (!silent) {
      AppEnv.showErrorDialog({
        ...
        message: localized(
          `Courier is clearing its cache %@. Depending on the size of the mailbox, this may take a few seconds or a few minutes. An alert will appear when cleanup is complete.`,
    ...
    try {
      ...
      if (!silent) {
        AppEnv.showErrorDialog({
          ...
          message: localized(
            `Courier reset the local cache for %@ in %@ seconds. Your mailbox will now begin to sync again.`,
            account.emailAddress,
      ...
      AppEnv.showErrorDialog({
        ...
        message: localized(`Courier was unable to reset the local cache. %@`, error),
      });
    } finally {
